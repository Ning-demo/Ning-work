...
function addTodoList(event: { keyCode: number; }) {
	if (!event || event.keyCode === 13) {
		// 首先获取input框的输入内容
		//@ts-ignore
		let inputValue: string = document.getElementsByClassName("txt")[0].value;
		if (inputValue) {
...
function circulationHtml(date: TodoListArray, checkId?: number) {
	...
	date.forEach((item) => {
		...
		li.innerHTML = ` 
          ...
          <a class= ${item.id == checkId && item.checked ? ' completed ' : item.checked ? "need-completed button-close " : "button-close"}  id=${item.id}  onclick=deleteDate(this)>X</a>
        `;
		ul.append(li)
	});
	changeLeftItem(date)
...
function changeLeftItem(data: TodoListArray) {
	let docuemnt: HTMLElement | null = document.getElementById('num')
	if (docuemnt) {
 ...
 */
 ...
 function toBeCompleted() {
    let result: ArrayItem[] = todoArray.filter((item: ArrayItem) => item.checked==false)
    circulationHtml(result)
